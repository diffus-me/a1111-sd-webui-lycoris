...
import l_network_lora as network_lora
import l_network_hada as network_hada
import l_network_ia3 as network_ia3
import l_network_lokr as network_lokr
import l_network_full as network_full
import l_network_norm as network_norm
import l_network_glora as network_glora
import l_network_oft as network_oft
...
def convert_diffusers_name_to_compvis(key, is_sd2):
    ...
    if match(m, r"lora_te(\d*)_embeddings_(.+)_embedding"):
        if m[0] != "":
            return f"1_model_{m[1]}_embedding"
        if is_sd2:
            return f"model_{m[1]}_embedding"

        return f"transformer_text_model_{m[1]}_embedding"
